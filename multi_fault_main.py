...
def simulation(graph_circuit, list_input_node, list_output_node, symbol_table_nodes, dict_levelization, tv, fault_enable=False, fault_values=None):
    """
...
def simulation_fault(graph_circuit, list_input_node, list_output_node, symbol_table_nodes, dict_levelization, tv, faults):
    """
    ...
    res_fault = simulation(graph_circuit, list_input_node, list_output_node, symbol_table_nodes, dict_levelization, tv,
                           fault_enable=True, fault_values=faults)
...
def main():
    ...
    file_names = ["file_name.bench"]
    ## ###############################


    # parsing the circuit
    graph_circuit, symbol_table_nodes, list_input_node, list_output_node, fault_dict, dict_levelization = circuit_parsing(file_names[0], True, True)

    ## ###############################
    # add one or more faults here (e.g. 2 faults: a-0, k-j-1)
    faults = ["a-0", "k-j-1"]
    ## ###############################

    res = []
    num_input_bits = len(list_input_node)
    for i in range(2**num_input_bits):
        tv = format(i, f'0{num_input_bits}b')
        # Any TV any fault
        # run simulation
        detect, res_no_fault, res_fault = simulation_fault(graph_circuit, list_input_node, list_output_node,
                                                           symbol_table_nodes, dict_levelization, tv, faults)
        # print result of fault simulation
        if detect:
            res.append(tv)
